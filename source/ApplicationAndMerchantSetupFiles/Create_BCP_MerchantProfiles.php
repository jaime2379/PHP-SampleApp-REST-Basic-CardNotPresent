...
$i = 0;
foreach ( $_serviceId as $service ) {
	
	$svcId = $service ['ServiceId'];
	$response = $client->getMerchantProfiles ( $svcId, 'Credit' ); // Note a empty value in ServiceId will allow you to pull all profiles across all services
	if (isset ($response[0])){
		foreach ($response as $profile){
			$merchantProfiles [$i] ['ProfileId'] = $profile->id;
			$merchantProfiles [$i] ['ServiceId'] = $svcId;
			$i++;
		}
...
/*
 * When using REST, getMerchantProfiles returns a list of ProfileIds. GetMerchantProfile
 * 		must then be called on one of the specific IDs. 
 * When using SOAP, getMerchantProfiles returns a list of MerchantProfile objects.
 */
if (is_array ( $response ) && isset ( $response [0] ) && !isset ( $merchantProfiles [1] )) {
	$response = $client->getMerchantProfile ( $_serviceId [0]->ServiceId, $response [0]->id );
}
